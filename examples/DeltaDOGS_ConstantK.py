        ...
        for k in range(iter_max):
            ...
            if xU.shape[1] != 0:
                for ii in range(xU.shape[1]):
                    yu[0, ii] = interpolation.interpolate_val(xU[:, ii], inter_par) - K * Utils.mindis(xU[:, ii], xE)[0]
            else:
                yu = np.array([[]])

            xi, ind_min = cartesian_grid.add_sup(xE, xU, ind_min)
            xc, yc, result = constantK_snopt_safelearning.triangulation_search_bound_snopt(inter_par, xi, K, ind_min, y_safe, L_safe)
            ...
            Dogsplot_sl.print_summary(num_iter, k, kk, xE, yE, summary, 1)
